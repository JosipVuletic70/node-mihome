...
class MiCloudProtocol {
  ...
  async request(path, data, country = 'de') {
    if (!this.isLoggedIn) {
    ...
    if (country !== 'de') console.log('ZEMLJA NIJE NJEMACKA', country);
  ...
  async miioCall(deviceId, method, params, options = {}) {
    ...
    console.log('DATA', data, 'DATA.RESULT', data.result);
    return data.result;
  }
...
module.exports = MiCloudProtocol;
