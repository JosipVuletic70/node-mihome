...
module.exports = class MiioDevice extends EventEmitter {
  ...
  async send(method, params, options = {}) {
    const result = this.protocol === 'cloud' ? await this.cloudSend(method, params, options) : await this.localSend(method, params, options);
    return result;
  }
  ...
  async loadProperties(props) {
    try {
      ...
      for (const propChunk of propsChunks) {
        ...
        if (!resultChunk) {
          throw new Error(`Properties is empty PropChunk ${propChunk}, Result Chunk ${resultChunk}`);
        }
  ...
  async miotGetProperties(props) {
    ...
    const params = props.map(prop => {
      ...
      console.log(this._miotSpec[prop]);
  ...
  async miotSetProperty(prop, value, options = {}) {
    ...
    if (!result || !result[0] || result[0].code !== 0) {
      throw new Error(`Could not perform operation RESULT: ${result}`);
    }
